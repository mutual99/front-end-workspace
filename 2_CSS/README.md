...
- 특정 요소를 선택해서 원하는 "스타일"과 "기능"을 적용 시킬 수 있음
...
- !important > 인라인 방식(style) > 아이디선택자 > 클래스 선택자 > 태그 선택자
...
- #기호가 다음에 6자리 16진수로 표시하는 방법
- 6자리는 앞에서부터 두자리씩 묶어서 Red, Green, Blue포 표시
- #000000(검은색) ~ #ffffff(휜색)
...
- 속성값이 static을 제외한 나머지들은 좌표를 이용해서 요소의 위치를 조정할 수 있다.
- 위치는 top, bottom, left, right로 지정할 수 있다.

# 반응형 웹

- 구간 별 페이지 제작 : PC, Tablet, Mobile
- viewport(스마트 폰 화면에서 실제 내용이 표시되는 영역) 작성

```html
<meta name="viewport" content="속성1=값, 속성2=값, ...." />
```

- width : viewport 너비 (device-width or 크기)
- height : viewport 높이 (device-height or 크기)
...
```css
.container {
  align-content: stretch or flex-start or flex-end or center or space-around or
...
- columns의 개수를 미리 정하지 않고 설정된 너비가 허용하는 한 최대한 셀을 채용
- auto-fill은 셀의 개수가 모자라면 공간이 남지만, auto-fill은 공간을 채움
...
## grid-column-start, grid-colimn-end, grid-column, grid-row-start, grid-row-end, grid-row
